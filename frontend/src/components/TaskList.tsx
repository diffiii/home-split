...
const TaskList: React.FC<TaskListProps> = ({
  ...
  const handleRemoveCompleted = async () => {
    ...
    if (!window.confirm(`Are you sure you want to delete ${completedTasks.length} completed task${completedTasks.length === 1 ? '' : 's'}? This action cannot be undone.`)) {
      return;
    }
    ...
    try {
      ...
      alert('Failed to delete some completed tasks. Please try again.');
    }
  };
