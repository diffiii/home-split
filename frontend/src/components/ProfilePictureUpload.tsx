    ...
    <div className="flex flex-col items-center space-y-4">
      ...
      {selectedFile ? (
        <div className="flex space-x-2">
          <Button onClick={handleUpload} variant="primary" size="sm" disabled={isLoading}>
            {isLoading ? 'Uploading...' : 'Save'}
          </Button>
          <Button onClick={handleCancel} variant="secondary" size="sm" disabled={isLoading}>
            Cancel
          </Button>
        </div>
      ) : (
      ...
      {selectedFile && <p className="text-xs text-gray-500 text-center">{selectedFile.name}</p>}
    </div>
  );
};
