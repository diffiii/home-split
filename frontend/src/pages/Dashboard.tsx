...
const Dashboard: React.FC = () => {
  ...
  const fetchActiveHouseholds = async (rawMemberships: RawMembership[]): Promise<Household[]> => {
    ...
    return (await Promise.all(householdPromises))
      .filter((household): household is Household => household !== null);
  };

  const fetchPendingInvitations = async (rawMemberships: RawMembership[]): Promise<Membership[]> => {
    ...
    <div className="min-h-screen bg-white">
      ...
      <main className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        ...
        <CreateHouseholdSection 
          ...
          onFormChange={{ setName: setNewHouseholdName, setDescription: setNewHouseholdDescription }}
          error={error}
        />
...
const WelcomeSection: React.FC<WelcomeSectionProps> = ({ username, householdsCount, pendingCount }) => (
  <div className="bg-white border border-gray-200 rounded-lg p-4 sm:p-6 lg:p-8 mb-6 sm:mb-8">
    <div className="flex flex-col md:flex-row md:items-center justify-between">
      ...
      <div className="mt-4 md:mt-0">
        <div className={`grid ${pendingCount > 0 ? 'grid-cols-2' : 'grid-cols-1'} gap-3 sm:gap-4 text-center`}>
...
const CreateHouseholdSection: React.FC<CreateHouseholdSectionProps> = ({
  showForm, onToggleForm, onSubmit, onReset, formData, onFormChange, error
}) => (
  <div className="mb-8">
    {!showForm ? (
...
const CreateHouseholdForm: React.FC<CreateHouseholdFormProps> = ({
  onSubmit, onCancel, formData, onFormChange, error
}) => (
...
const HouseholdsGrid: React.FC<HouseholdsGridProps> = ({ 
  households, pendingInvitations, onViewHousehold, onHouseholdUpdated, onShowCreateForm 
}) => {
